  ...
  try {
    ...
    if (assignedTo) {
      await createNotification({
        ...
        message: `"${title}" has been assigned as part of "${parent.title}"`,
        type: 'task',
        priority: priority || 'medium',
        actionUrl: `/tasks/${subtask._id}`,
...
const requestCompletion = async (req, res) => {
  try {
    ...
    // Notify the manager (assignedBy)
    if (task.assignedBy && task.assignedBy.toString() !== userId.toString()) {
      await createNotification({
        organizationId: orgId,
        recipient: task.assignedBy,
        title: '✅ Task Awaiting Approval',
        message: `"${task.title}" has been submitted for completion review.`,
        type: 'task',
        priority: 'high',
        actionUrl: `/tasks/approvals`,
